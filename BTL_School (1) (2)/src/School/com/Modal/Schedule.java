...
public class Schedule implements Comparable<Schedule>{
    ...
    public void display(){
        for(int i=0;i<classes.size();i++)
            System.out.println(classes.get(i).getStudentGroup().getId()+" "+classes.get(i).getRoom().getId()+" "+classes.get(i).getTeacher().getId()+" "+classes.get(i).getCourse().getId()+" "+classes.get(i).getStudentGroup().getName()+" "+classes.get(i).getClassTime().toString());
    }
    public ClassModel mutation(ArrayList<Room>rooms,ArrayList<Teacher> teachers,HashMap<String,Course>courses,StudentGroup studentGroup){

        Random rd=new Random();
        int rd_room=rd.nextInt(rooms.size());
        int rd_teacher=rd.nextInt(teachers.size());
        return (new ClassModel(teachers.get(rd_teacher),rooms.get(rd_room),studentGroup, courses.get(studentGroup.getIdCourse())));
    }
    public double calFitness(){
        ...
        for(int i=0;i<classes.size()-1;i++){
            ...
            for(int j=i+1;j<classes.size();j++){
                ...
                if ((classes.get(i).getStudentGroup().getId() == null && classes.get(j).getStudentGroup().getId() == null) ||
    (classes.get(i).getStudentGroup().getId() != null && classes.get(i).getStudentGroup().getId().equals(classes.get(j).getStudentGroup().getId())) ||
    (classes.get(i).getStudentGroup().getName().equals(classes.get(j).getStudentGroup().getName()))) {
    if (!classes.get(i).getClassTime().check(classes.get(j).getClassTime())) {
        number++;
    }
}
